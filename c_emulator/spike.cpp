...
void spike::reflect_spike_csr(reg_t csr_addr)
{
  sbits sail_val;
  // set sail_val.len correctly
  sail_val = zread_CSR(csr_addr);
  // now set the bits to match Spike
  sail_val.bits = get_csr_unchecked(csr_addr);
  zwrite_CSR(csr_addr, sail_val);
}
...
void spike::setup_model()
{
  ...
  // MIP isn't accessible by read_CSR, so inline
  // `reflect_spike_csr` here.
  sbits sail_val;
  sail_val.len = zmip.zbits.len;
  sail_val.bits = get_csr_unchecked(CSR_MIP);
  zmip = zMk_Minterrupts(sail_val);

  // Match the default Spike PMP config, which only sets the first
  // entry to permit unprivileged access to all memory.
  reflect_spike_csr(CSR_PMPADDR0);
  reflect_spike_csr(CSR_PMPCFG0);
}
