                        ...
                        lbits value)
{
  ...
  for (auto c : callbacks) {
    c->mem_write_callback(type, paddr.bits, width, value);
  }
  return UNIT;
}
                       ...
                       lbits value)
{
  ...
  for (auto c : callbacks) {
    c->mem_read_callback(type, paddr.bits, width, value);
  }
  return UNIT;
}
...
unit mem_exception_callback(sbits paddr, uint64_t num_of_exception)
{
  ...
  for (auto c : callbacks) {
    c->mem_exception_callback(paddr.bits, num_of_exception);
  }
  return UNIT;
}
                              ...
                              sbits value)
{
  ...
  for (auto c : callbacks) {
    c->xreg_full_write_callback(abi_name, reg, value.bits);
  }
  return UNIT;
}
...
unit freg_write_callback(unsigned reg, sbits value)
{
  ...
  for (auto c : callbacks) {
    c->freg_write_callback(reg, value.bits);
  }
  return UNIT;
}
                             ...
                             sbits value)
{
  ...
  for (auto c : callbacks) {
    c->csr_full_write_callback(csr_name, reg, value.bits);
  }
  return UNIT;
}
                            ...
                            sbits value)
{
  ...
  for (auto c : callbacks) {
    c->csr_full_read_callback(csr_name, reg, value.bits);
  }
  return UNIT;
}
...
unit pc_write_callback(sbits value)
{
  (void)value;
  for (auto c : callbacks) {
    c->pc_write_callback(value.bits);
  }
  return UNIT;
}
